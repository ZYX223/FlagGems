...
def is_sqmma_compatible(a, b, N, K):
    ...
    fp32_sqmma = dot_mode == 2 and b.stride(0) == 1
    dtype_supported = a.dtype in (torch.float16, torch.bfloat16) or fp32_sqmma
    return (
        a.dim() == 2
        and b.dim() == 2
        and a.dtype == b.dtype
...
def addmm_dtype_out(bias, mat1, mat2, out_dtype, *, beta=1, alpha=1, out):
    ...
    if out_dtype == mat1.dtype and is_sqmma_compatible(mat1, mat2, N, K):
        result = addmm_sqmma(
            mat1,
            mat2,
            bias_c,
